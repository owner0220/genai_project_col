...
CHECKPOINT_PATH = os.path.join(os.path.expanduser("~"),"segment-anything-with-clip")
...
def segment(
    ...
    image_path: str,
    query: str,
) -> Tuple[PIL.ImageFile.ImageFile, List[PIL.Image.Image]]:
    ...
    # Sort masks by their top-left corner (x, y)
    masks.sort(key=lambda mask: (mask["bbox"][1], mask["bbox"][0]))
    
    # Get cropped images in sorted order
    cropped_images = [crop_image(image, mask) for mask in masks]
...
if __name__ == "__main__":
    ...
    # demo.launch(ssl_certfile="cert.pem", ssl_keyfile="key.pem",ssl_verify=False,server_name="0.0.0.0",server_port=7860)
